...
func black_fade(fade_in:float, hold_time:float, fade_out:float, hide_characters:bool,
 new_background:String, new_bgm:String, red:float, blue:float, green:float, wait_for_completion:bool):
...
func fade_character_in(character_name, fade_duration) -> bool:
	character_name = String(character_name)
	fade_duration = float(fade_duration)
	emit_signal("fade_character", character_name, fade_duration)
	return true
...
func apply_filter_room_one(visible) -> bool:
	...
	# (Needs to be called by your code from somewhere.)
	# (The most direct approach is Parser.line_reader.instruction_handler.instruction_completed.emit().)
	GameWorld.stage_root.apply_filter_room_one(visible)
	return false

func set_text_speed(value) -> bool:
	value = float(value)
	# Return true if you want the LineReader to wait until its InstructionHandler has emitted instruction_completed.
	# (Needs to be called by your code from somewhere.)
	# (The most direct approach is Parser.line_reader.instruction_handler.instruction_completed.emit().)
